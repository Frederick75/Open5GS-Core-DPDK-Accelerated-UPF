...
## 11. Carrier-grade multi-session support

The UPF maintains a **concurrent session table** keyed by **SEID** to support multiple sessions.
Uplink N3 classification uses UL TEID lookup; this reference uses a safe O(capacity) scan.
For production deployments, add a TEID→SEID index (hash map) for O(1) classification.
